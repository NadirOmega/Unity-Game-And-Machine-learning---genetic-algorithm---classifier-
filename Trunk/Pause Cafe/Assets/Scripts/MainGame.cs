			...
			if (pathWalk != null){
				...
				if (currentPlayerType == PlayerType.HUMAN){
					...
					if (Input.GetKeyDown(KeyCode.Z)){
						Debug.Log(AIHard.rules.getStringInfo());
						AIHard.rules.dispInFile("Data/rules.txt");
					}
					
					// TEST ALGO GEN
					if (Input.GetKeyDown(KeyCode.R)){
						Classifier c = new Classifier(hexaGrid,currentCharControlledID);
						Debug.Log(c.getStringInfo());
						AlgoGen.MutateHpCurrentChar(c);
						Debug.Log(c.getStringInfo());
					}
						...
						if (decisionSequence.Count == 0){
							switch (currentPlayerType){
								...
								case PlayerType.AI_HARD : {
									decisionSequence = AIHard.decide(currentCharControlledID,statsGame);
								} break;
							}
						}else{
							...
							switch (actionAIPos.action){
								...
								case ActionType.ATK1 : case ActionType.ATK2 : actionUseAttack(actionAIPos.action,hexaGrid.getHexa(actionAIPos.pos)); break;
