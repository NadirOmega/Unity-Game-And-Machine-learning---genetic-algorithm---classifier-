...
public class MainGame : MonoBehaviour {
    ...
    void Start(){
		...
		if (AIHard.rules == null) AIHard.rules = new ClassifierSystem();
			...
			if (pathWalk != null){
				...
				if (currentPlayerType == PlayerType.HUMAN){
					...
					if (Input.GetKeyDown(KeyCode.Z)){
						...
						AlgoGen.MutateThreatCurrentChar(classifier);
					...
					if (Input.GetKeyDown(KeyCode.E)){
						Debug.Log(AIHard.rules.classifiers.Count);
					}
				...
				// ACTIONS FOR AI PLAYERS
					...
					if (newTurn > AIwaitingTime){
						if (decisionSequence.Count == 0){
							...
							Debug.Log("AI : " + actionAIeasyPos.action + ((actionAIeasyPos.pos != null) ? (" " + actionAIeasyPos.pos.x + " " + actionAIeasyPos.pos.y) : ""));
