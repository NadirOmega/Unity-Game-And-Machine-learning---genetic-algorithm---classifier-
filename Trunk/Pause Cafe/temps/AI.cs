...
public class AIHard : AI{
	...
	public static List<ActionAIPos> decide(int charID,StatsGame statsGame){
		...
		if(true){
				...
				if (learn) {
					Debug.Log("SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS2");
					System.Random random = new System.Random();
					int nb = random.Next(0,4);
					if (nb==1 || nb==2 | nb==3){
						Debug.Log("SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS3");
						rule = Classifier.getClassifierBestAction(validRules,currentSituation);
					}
					else 
					rule = Classifier.getClassifierRouletteWheel(validRules);
				}
